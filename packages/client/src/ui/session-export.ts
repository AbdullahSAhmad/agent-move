...
import { AGENT_PALETTES, ZONE_MAP, getModelPricing } from '@agentflow/shared';
...
export class SessionExport {
  ...
  private generateReport(): string {
    ...
    // Build markdown
    const lines: string[] = [];
    lines.push(`# AgentFlow Session Summary`);
    ...
    lines.push('---');
    lines.push('*Generated by AgentFlow*');
  ...
  private download(): void {
    ...
    a.href = url;
    a.download = `agentflow-session-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.md`;
    a.click();
